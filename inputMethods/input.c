...
void initInput(uint16_t w, uint16_t h)
{
  ...
  if((inputfd = suinput_open(0, w, h)) == -1)
  {
    L("cannot create virtual kbd device.\n");
    sendMsgToGui("~SHOW|Cannot create virtual touch device!\n");
    //  exit(EXIT_FAILURE); do not exit, so we still can see the framebuffer
  }

  if((kbdfd = suinput_open(1, w, h)) == -1)
  {
    L("cannot create virtual kbd device.\n");
    sendMsgToGui("~SHOW|Cannot create virtual keyboard device!\n");
    //  exit(EXIT_FAILURE); do not exit, so we still can see the framebuffer
  }
...
void keyEvent(rfbBool down, rfbKeySym key, rfbClientPtr cl)
{
  ...
  if ((code = keysym2scancode(down, key, cl,&sh,&alt)))
  {
    ...
    if (key && down)
    {
      if (sh) suinput_press(kbdfd, 42); //left shift
      if (alt) suinput_press(kbdfd, 56); //left alt

      ret=suinput_press(kbdfd,code);
      ret=suinput_release(kbdfd,code);

      if (alt) suinput_release(kbdfd, 56); //left alt
      if (sh) suinput_release(kbdfd, 42); //left shift
    }
    else
...
//  transformTouchCoordinates(&x,&y,cl->screen->width,cl->screen->height);
  ...
  else if (buttonMask & 1)//left btn clicked
  {
    ...
    suinput_write(inputfd, EV_ABS, ABS_X, x);
    suinput_write(inputfd, EV_ABS, ABS_Y, y);
    suinput_write(inputfd, EV_ABS, ABS_PRESSURE, 51);
  ...
  else if (leftClicked)//left btn released
  {
    ...
    suinput_write(inputfd, EV_ABS, ABS_X, x);
    suinput_write(inputfd, EV_ABS, ABS_Y, y);
    suinput_write(inputfd, EV_ABS, ABS_PRESSURE, 0);
